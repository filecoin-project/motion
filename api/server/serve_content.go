...
type WriterToN interface {
	WriteToN(io.Writer, int64) (int64, error)
}
...
// serveContext is a copy of http.ServeContent with the minor modification that
// is can call content.WriteToN if available, and it does not sniff content to
// determine content type.
//
// See: https://pkg.go.dev/net/http#ServeContent
func serveContent(w http.ResponseWriter, r *http.Request, name string, modtime time.Time, content io.ReadSeeker) {
	...
	// If Content-Type isn't set, use the file's extension to find it. Do not
	// sniff content to find type. Default to "application/octet-stream" if no
	// content type is set.
	...
	case len(ranges) > 1:
		...
		go func() {
			for _, ra := range ranges {
				...
				wtn, ok := content.(WriterToN)
				if ok {
					if _, err := wtn.WriteToN(part, ra.length); err != nil {
						pw.CloseWithError(err)
						return
					}
				} else {
